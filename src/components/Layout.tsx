...
export function Layout({ vault, onVaultUpdate, onLock }: LayoutProps) {
  ...
  const handleSecretSubmit = async (name: string, value: string, folderId: string) => {
    if (editingSecret) {
      const updated = await api.updateSecret(editingSecret.id, name, value, folderId);
      onVaultUpdate({
        ...vault,
        secrets: vault.secrets.map((s) => (s.id === updated.id ? updated : s)),
      });
    } else {
      const newSecret = await api.addSecret(name, value, folderId);
      onVaultUpdate({
        ...vault,
