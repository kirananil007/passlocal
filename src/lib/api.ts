...
export async function addSecret(name: string, key: string, value: string, notes: string, folderId: string): Promise<Secret> {
  return invoke('add_secret', { name, key, value, notes, folderId });
}

export async function updateSecret(id: string, name: string, key: string, value: string, notes: string, folderId: string): Promise<Secret> {
  return invoke('update_secret', { id, name, key, value, notes, folderId });
}
